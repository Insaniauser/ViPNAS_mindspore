import mindspore
from mindspore.common.initializer import initializer, Normal, One
...
class ViPNASHeatmapSimpleHead(nn.Cell):
    ...
    def normal_init(module, mean=0, std=1., bias=0):
        ...
        if hasattr(module, 'gamma_init') and module.gamma_init is not None:
            module.gamma_init = initializer(Normal(sigma=std, mean=mean), module.gamma_init.shape, mindspore.float32)
        if hasattr(module, 'beta_init') and module.weight is not None:
    ...
    def constant_init(module, val, bias=0):
        ...
        if hasattr(module, 'beta') and module.weight is not None:
...
def resize(input,
           ...
           warning=True):
    if mode != 'bilinear':
        raise TypeError("only support bilinear mode")
    if warning:
    ...
    return ops.ResizeBilinear(input, size, scale_factor, align_corners)
...
def keypoints_from_heatmaps(heatmaps, center, scale):
    ...
    for n in range(N):
        for k in range(K):
            ...
            px = int(preds[n][k][0])
            py = int(preds[n][k][1])
            if 1 < px and px < W - 1 and 1 < py and py < H - 1:
                diff = np.array([
