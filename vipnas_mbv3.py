...
from mindspore.common.initializer import initializer, Normal, HeNormal, HeUniform, Constant, One


class ViPNAS_MobileNetV3(nn.Cell):
    ...
    def __init__(self,
        ...
        super(ViPNAS_MobileNetV3, self).__init__()
        self.wid = wid
        self.expan = expan
        self.dep = dep
        self.ks = ks
        ...
        for _, m in self.cells_and_names():
            if isinstance(m, nn.Conv2d):
                m.weight = initializer(
                    ...
                    m.weight.shape,
                    mindspore.float32)
                for parameter in m.get_parameters():
                    if 'bias' in parameter.name:
                        m.bias = initializer(0, m.bias.shape, mindspore.float32)
    ...
    def construct(self, x):
        ...
        for block in self.cells_and_names():
            if 'layer' in block[0] and '.' not in block[0]:
                x = block[1].construct(x)
...
class ConvModule(nn.Cell):
    ...
    def __init__(self,
        ...
        if set(order) != set(['conv', 'norm', 'act']):
        ...
        if self.with_norm:
            ...
            self.insert_child_to_cell('norm', norm)
        else:
            self.norm = None
...
def build_activation_layer(act_cfg):
    if act_cfg.get('type') == 'ReLU':
        return nn.ReLU()
    elif act_cfg.get('type') == 'HSwish':
        return nn.HSwish()
    elif act_cfg.get('type') == 'Sigmoid':
        return nn.Sigmoid()
    elif act_cfg.get('type') == 'HSigmoid':
        return HSigmoid()
